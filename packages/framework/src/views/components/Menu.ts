...
export class Menu extends LitElement {
    ...
    private handlePresetClick(item: MenuPresetItem) {
        this.handlers.loadPreset(item);
        this.isOpen = false;
        document.removeEventListener('click', this.boundClickOutside);
    }

    private handleActionClick(item: MenuItem) {
    ...
    private renderMenuItem(item: MenuItem, depth: number = 0): TemplateResult | typeof nothing {
        if (item.type === 'parent') {
            return this.renderParentItem(item as MenuParentItem, depth);
        }
        if (item.type === 'preset') {
            return this.renderPresetItem(item as MenuPresetItem, depth);
        }
        if (item.type === 'action') {
    ...
    private renderIcon(icon: string | undefined, fallbackType: MenuItem['type']) {
        switch (icon) {
            case 'designer':
                return html`
    ...
    private renderParentItem(item: MenuParentItem, depth: number): TemplateResult {
    ...
    private renderPresetItem(item: MenuPresetItem, depth: number) {
        const isDragging = this.draggedItemId === item.id;
        const isDropTarget = this.dropTargetId === item.id;
    ...
    private renderActionItem(item: MenuItem, depth: number) {
        const isDragging = this.draggedItemId === item.id;
        const isDropTarget = this.dropTargetId === item.id;
    ...
    render() {
        ...
        return html`
            ...
            ${this.isOpen ? html`
                <div class="menu" @click=${this.handlers.stopClickPropagation}>
...
customElements.define('menu-view', Menu);
