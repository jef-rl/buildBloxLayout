import { Panel, View } from "../types/panels.types";
import { UIState, LayoutState } from "../types/state.types";
...
export const applyMainViewOrder = (state: UIState, viewOrder: string[]): UIState => {
    ...
    const nextPanels = state.panels.map((panel: Panel) => {
        ...
        if (definition) {
             ...
             } as Panel & { _pendingCreation?: boolean; _definitionId?: string };
        }
    ...
    const activePanelViews = finalPanels.map((p: Panel) => p.view).filter((v): v is View => Boolean(v));
...
const assignViewToPanel = (
    ...
    const sourcePanel = state.panels.find((p: Panel) => p.viewId === instanceId || p.activeViewId === instanceId);
    ...
    let nextPanels = state.panels.map((p: Panel): Panel => {
        if (p.id === panel.id) {
            return nextPanel;
        }
...
function assignViewHandler(state: UIState, action: { payload: { panelId: string; viewId: string; data?: unknown; placement?: "top" | "bottom"; swap?: boolean; } | undefined; }) {
...
function removeViewHandler(state: UIState, action: { payload: { panelId: string; } | undefined; }) {
    ...
    const nextPanels = state.panels.map((p): Panel => {
