import { generateAuthMenuItems } from '../helpers/auth-menu-items.utils';
import { migrateLegacyExpansion, LegacyLayoutExpansion } from '../helpers/expansion-helpers.utils';
import { frameworkMenuPersistence } from '../framework/framework-menu-persistence.utils';
import { HandlerAction, ReducerHandler, HandlerRegistry, HandlerResult } from '../registries/handler.registry';
import { FRAMEWORK_ADMIN_EMAILS } from '../helpers/helpers.utils';
import { dragHandlers } from '../layout/layout-drag.handlers';
import { viewInstanceHandlers } from '../layout/layout-view-instances.handlers';
import { UIState, LayoutExpansion, LayoutPreset, LayoutPresets, FrameworkMenuConfig } from '../types/state.types';
...
export const registerWorkspaceHandlers = (
  ...
  registerHandler(registry, 'panels/assignView', assignViewHandler);
  registerHandler(registry, 'panels/removeView', removeViewHandler);
...
function assignViewHandler(state: FrameworkContextState, action: HandlerAction<Record<string, unknown>>): HandlerResult<FrameworkContextState> {
  throw new Error('Function not implemented.');
}

function removeViewHandler(state: FrameworkContextState, action: HandlerAction<Record<string, unknown>>): HandlerResult<FrameworkContextState> {
  throw new Error('Function not implemented.');
}
