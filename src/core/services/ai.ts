...
export type AiSuccess = { status: 'success'; text: string; fullResponse?: unknown };
export type AiError = { status: 'error'; error: string; fullResponse?: unknown };
export type AiResponse = AiSuccess | AiError;
export type AiClient = (prompt: string, systemInstruction?: string) => Promise<AiResponse>;
...
export const noopAiClient: AiClient = async () => {
  ...
  return {
    status: 'success',
    text: 'AI client not configured. Did you forget to inject a real client?',
  };
};
...
export const liveAiClient: AiClient = async (prompt, systemInstruction) => {
  ...
  try {
    ...
    const message =
      e instanceof DOMException && e.name === 'AbortError'
        ...
        : 'Network error calling AI API.';
    console.error('AI API network error:', e);
    return { status: 'error', error: message, fullResponse: e };
  } finally {
  ...
  try {
    ...
    if (!text) {
      console.error('Invalid AI API response:', result);
      return { status: 'error', error: 'Invalid AI API response.', fullResponse: result };
    }

    return { status: 'success', text, fullResponse: result };
  } catch (e) {
    console.error('Error parsing AI API response:', e);
    return { status: 'error', error: 'Error parsing AI API response.', fullResponse: e };
  }
};
...
export function createGeminiClient(apiKey: string): AiClient {
  ...
  return async (prompt: string, systemInstruction?: string) => {
    ...
    try {
      ...
      return { status: 'error', error: `Error calling Gemini API: ${e.message}` };
    }
  };
}
