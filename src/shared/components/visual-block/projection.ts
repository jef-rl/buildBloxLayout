...
import type { ProjectionDragStart, UiDispatch, UiStateContextValue } from '../../../core/state/ui-state.js';
...
export class VisualBlockProjection extends LitElement {
  ...
  private _onMouseDown(e: MouseEvent) {
    ...
    const dragStart: ProjectionDragStart = { x: e.clientX, initialRotation: startRot };
    this.uiDispatch?.({ type: 'projection/drag-start', dragStart });
    window.addEventListener('mousemove', this._handleMouseMove);
    window.addEventListener('mouseup', this._handleMouseUp);
  }

  private _onMouseMove(e: MouseEvent) {
    const dragStart = this.uiState?.interaction?.projection?.dragStart;
    if (!dragStart) return;
    const deltaX = e.clientX - dragStart.x;
    const newRot = Math.max(-70, Math.min(70, dragStart.initialRotation + deltaX * 0.2));
    this.dispatchUiEvent('projection-rotate', newRot);
  }

  private _onMouseUp() {
    this.uiDispatch?.({ type: 'projection/drag-end' });
