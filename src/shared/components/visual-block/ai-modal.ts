...
import { uiStateContext } from '../contexts.ts';
 ...
 *
 * Responsibility:
 * - View overlay UI bound to provider-owned `view` state (via uiStateContext).
...
export class VisualBlockAiModal extends LitElement {
  private uiState: any = { view: { activeView: '' } };
  private uiDispatch?: UiStateContextValue['dispatch'];
  ...
  private dispatchModalEvent(type: string, payload: any = null) {
    this.dispatchEvent(new CustomEvent(type, { detail: payload, bubbles: true, composed: true }));
  }

  private handleSubmit() {
    const viewState = this.uiState?.view ?? { activeView: '' };
    const payload = viewState.viewPayload ?? {};
    const isResult = payload.mode === 'result';
    const input = this.shadowRoot?.querySelector<HTMLTextAreaElement>('textarea')?.value ?? '';

    if (!isResult && !input.trim()) return;

    this.uiDispatch?.({ type: 'modal/loading', loading: true });
    const submitPayload = { mode: payload.mode, input: input.trim(), contextId: payload.contextId };
    this.dispatchUiEvent('modal-submit', submitPayload);
    this.dispatchModalEvent('modal-submit', submitPayload);

    // UI nicety: reset local loading quickly; provider owns the real async work
    setTimeout(() => this.uiDispatch?.({ type: 'modal/loading', loading: false }), 400);
  }

  private close() {
    this.dispatchUiEvent('modal-close');
    this.dispatchModalEvent('modal-close');
  }

  render() {
    const viewState = this.uiState?.view ?? { activeView: '' };
    const payload = viewState.viewPayload ?? {};
    const open = viewState.activeView === 'ai-prompt';
    const loading = this.uiState?.interaction?.modal?.loading ?? false;

    const isResult = payload.mode === 'result';
    const placeholder =
      payload.mode === 'architect'
        ? 'e.g., Add a row of 3 feature cards with icons...'
        : 'e.g., Make it punchier, fix grammar...';

    return html`
      <div class=${classMap({ overlay: true, open })}>
        <div class="dialog">
          <h3>${Icons.Sparkles} ${(viewState.viewOptions?.title as string) || 'AI Assistant'}</h3>

          ${isResult
            ? html`<div class="result-box">${payload.content}</div>`
            : html`<textarea placeholder=${placeholder} ?disabled=${loading}></textarea>`}

          <div class="actions">
            <button class="cancel" @click=${() => this.close()}>Close</button>
            ${!isResult
              ? html`
                  <button class="generate" @click=${() => this.handleSubmit()} ?disabled=${loading}>
                    ${loading ? html`<div class="spinner"></div>` : html`${Icons.Magic} Generate`}
                  </button>
                `
              : null}
          </div>
