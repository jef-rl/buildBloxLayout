...
Views can emit structured logs through the built-in log handler. This keeps logging consistent and visible in the `log-view` panel.
...
To display logs, register the framework `LogView` and assign it to a panel:

```typescript
import { LogView } from '@project/framework';
...
// View registration
{
  ...
  tag: 'log-view',
  icon: 'terminal',
  component: () => import('@project/framework').then(m => m.LogView)
}
```
...
## Integration Guide (Minimal Surface)

This guide focuses on the smallest set of framework APIs needed to integrate views correctly while honoring the core principles (pure handlers, immutable updates, unidirectional flow).

### 1) Register Your Views

Use `bootstrapFramework` with an array of view definitions. Each view definition maps a stable `id` to a custom element tag and lazy loader.
...
### 2) Consume Context (Read-Only)

Views should read state via `uiStateContext` and `ContextConsumer`. Treat the state as immutable and read-only.
...
Dispatch actions with `dispatchUiEvent`. Never mutate state directly from a view.
...
### 4) Register Custom Handlers (Pure Functions Only)

For app-specific behavior, register handlers that synchronously return new state and optional follow-up actions.
...
### 5) Optional: Use Framework Logging

Log from views via `logs/append` to keep logging centralized and visible in `log-view`.
...
Stick to these APIs unless you are extending the framework itself:

- `bootstrapFramework` to register views and mount the root
- `dispatchUiEvent` to request state changes
- `uiStateContext` + `ContextConsumer` to read state
- `frameworkHandlers.register` for app-specific behavior (pure, synchronous)
- `log-view` + `logs/append` for optional logging
